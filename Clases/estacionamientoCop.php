    ...
    public function IngresarVehiculo()//Verificado --> Probar funcionalidad.
    {
        ...
        if($validacion == 0)
        {
            ...
            */
            ...
            if($resultado)
            {
                //registrarOperacion($this->_empleado,'alta',date('Y-m-d'),$this->_numCochera);//Verificar
                echo "Entro al if RESULTADO";
        ...
        /*
                  $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
                  $consulta =$objetoAccesoDato->RetornarConsulta("INSERT into operaciones (numCochera,esDisca,ocupada,marca,patente,color,foto,id_empleado_ingreso,fecha_hora_ingreso,id_empleado_ingreso,tipo)values(:numCochera,:esDisca,:ocupada,:marca,:patente,:color,:foto,:id_empleado_ingreso,:fecha_hora_ingreso,:id_empleado_ingreso,:tipo)");
